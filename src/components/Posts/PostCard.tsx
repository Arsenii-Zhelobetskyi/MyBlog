...
function PostCard({
  ...
  post: { id: number; cover: string; title: string; created_at: string, likes:number };
}) {
  const navigate = useNavigate();
  const formattedDate = format(new Date(post.created_at), 'MMM dd, yyyy');
  return (
    <Card
      className="min-h-96 w-80 cursor-pointer bg-background transition-colors hover:bg-accent hover:text-accent-foreground"
      key={post.id}
